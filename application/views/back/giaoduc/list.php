...
<div class="contentcontainer">
    <div class="headings altheading">
        ...
        <h2>Danh sách tin
            <a class="add" href="<?= site_url('back/giaoduc/add'.'/'.$type)?>"><img src="<?= base_url('theme/img/add.png') ?>" width="25" height="25" style="vertical-align:middle; border:0px" /> Thêm Mới</a>
        </h2>   
            ...
            <?php foreach ($list as $row) { ?>
                <tr class="hang">
                    ...
                    <td style="text-align: center" class="action">
                        <a href="<?= site_url('back/giaoduc/edit/'.$type . '/' . $row->id) ?>">Chỉnh</a>
                        <?php if($type==3 ||$type==4){ ?>
                      |  <a href="<?=  base_url('back/giaoduc/delete/'.$type . '/' . $row->id)?>" onclick="return confirm('Có chắc bạn muốn xóa ?');" >Xóa</a> |
                        
                        <img src="AnHien_<?= $row->article_status ?>.png" width="15" height="15" title="" class="anhien" id="<?= $row->id ?>" />  <?php }?>
