            ...
            <p>
                <select name="check_category" id="check_category" style="width:350px;height: 35px">
                    ...
                    <?php foreach ($this->M_category->list_category_all(array('category.category_top' => 0, 'category.category_type' => 1),'vn') as $row) { ?>
                        ...
                        <option style="color:#f00" <?php if($this->M_category->count_category($row->id) > 0){ ?> disabled="disable"<?php }?> value="<?=$row->id?>"  <?php if ($this->M_artice->category_dangbo($id,$row->id) >0) { ?> selected="selected"<?php } ?> ><?=$row->category_name?></option>
                    <?php foreach ($CI->M_category->list_category_chill($row->id,'vn') as $dmcon) { ?>
                       <option value="<?=$dmcon->id?>" <?php if ($this->M_artice->category_dangbo($id,$dmcon->id) >0) { ?> selected="selected"<?php } ?>>|--<?=$dmcon->category_name?></option>
	...
	<div class="contentbox">
        ...
        <p>
           ...
           <p>
               ...
               <input type="radio" value="1" name="hot" <?php if($edit->article_hot==1) {?> checked="checked" <?php }?>>Hiển thị
               <input type="radio" value="0" name="hot"<?php if($edit->article_hot==0) {?> checked="checked" <?php }?> >Ẩn
           </p>
