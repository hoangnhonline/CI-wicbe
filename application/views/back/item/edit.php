...
<form action="" method="post" enctype="multipart/form-data" id="formNewsDetail">
    <div class="contentcontainer med left">
        ...
        <div class="contentbox">
            ...
            <p style="display: none">
                ...
                <input name="code" value="<?=$item->code?>"  type="text" id="textfield" class="inputbox">

            </p>
            <p >
                <label for="textfield"></strong></label>
                <input name="name"  type="text" id="textfield" value="<?=$item->name?>" class="inputbox">
                <?php echo form_error('name'); ?>
            </p>
            <p>
                <label for="textfield"><strong>Tóm Tắt </strong></label>
                <textarea  style="height: 100px" name="summary" class="textarea inputbox" ><?=$item->summary?></textarea>
            </p>

            <p>
                <label for="textfield"><strong>Thông tin sản phẩm </strong></label>
                <textarea class="textarea" cols="80" id="content" name="content" rows="15"><?=$item->content?></textarea>
            </p>
            <p>
                <label for="textfield"><strong>Chi tiết </strong></label>
                <textarea class="textarea" cols="80" id="content1" name="content1" rows="15"><?=$item->content1?></textarea>
            </p>
            <p>
            ...
            <div class="panel-body">
                <select class="tokenize-sortable-demo1" multiple>
                    <?php foreach ($CI->M_tags->show_list_tags() as $row1){ ?>
                        <?php $check = $CI->M_item->check_tags(array('id'=>$item->id,'id_tags'=>$row1->id,'type'=>1)); ?>
                        <option <?php if($check==1){?> selected <?php } ?> value="<?=$row1->id?>"><?=$row1->name?></option>
                    <?php } ?>
        ...
        <div class="contentbox">
            ...
            <p>
                <select name="cate" id="check_category" style="width:350px;height: 35px;display: block !important;">
                    ...
                    <?php foreach ($this->M_category->list_category_all(array('top' => 0, 'type' => 1)) as $row) { ?>
                        <option style="color:#f00" <?php if($row->id==$item->cate) {?>selected <?php } ?> value="<?=$row->id?>"><?=$row->name?></option>
                        <?php foreach ($this->M_category->list_category_chill($row->id) as $dmcon) { ?>
                            <option value="<?=$dmcon->id?>" <?php if($dmcon->id==$item->cate) {?>selected <?php } ?>>|----<?=$dmcon->name?></option>
                        <?php }?>
                    <?php }?>
                </select>
            </p>
    ...
    <div class="contentcontainer sml right">
        ...
        <div class="contentbox">
            ...
            <p>
                ...
                <input name="title"  type="text" id="textfield" value="<?=$item->title?>" class="inputbox weight">
            </p>

            <p>
                <label for="textfield"><strong>Keywords </strong></label>
                <textarea  style="height: 100px" name="keywords" class="textarea inputbox weight" ><?=$item->keywords?></textarea>
            </p>
            <p>
                <label for="textfield"><strong>Description </strong></label>
                <textarea  style="height: 100px" name="description" class="textarea inputbox weight" ><?=$item->description?></textarea>
            </p>
    ...
    <div class="contentcontainer sml right">
        ...
        <div class="contentbox">
            <p style="width:40%; float: left">
                ...
                <input type="radio" value="1" name="status" <?php if($item->status==1){?>checked<?php }?>>Hiển thị
                <input type="radio" value="0" name="status" <?php if($item->status==0){?>checked<?php }?>>Ẩn
            </p>

            <p style="width:40%; float: left;">
                <label for="smallbox"><strong>Nổi Bật: </strong></label>
                <input type="radio" value="1" name="hot" <?php if($item->hot==1){?>checked<?php }?>>Có
                <input type="radio" value="0" name="hot" <?php if($item->hot==0){?>checked<?php }?>>Không
            </p>

            <p style="clear:both"></p>
            <p>
                <label for="textfield"><strong>Hot</strong></label>
                <input name="name_hot"  type="text" id="textfield" value="<?=$item->name_hot?>" class="inputbox weight">
            </p>
            <p>
                <label for="textfield"><strong>Thứ tự</strong></label>
                <input name="weight"  type="text" id="textfield" value="<?=$item->weight?>" class="inputbox weight">
            </p>
            <p>
                <label for="textfield"><strong>Giá gốc</strong></label>
                <input name="price"  type="text" id="textfield" value="<?=$item->price?>" class="inputbox weight">
            </p>
            <p>
                <label for="textfield"><strong>Giá khuyến mãi</strong></label>
                <input name="price_pro"  type="text" id="textfield" value="<?=$item->price_pro?>" class="inputbox weight">
            </p>
    ...
    <div class="contentcontainer sml right">
        ...
        <div class="contentbox">
            ...
            <img src="<?= base_url() ?>timthumb.php?src=<?= base_url() ?><?=$item->img ?>&w=90&h=90&zc=1">
            <span id="imsssg">
            ...
            <?php foreach ($this->M_item->other_img($item->id) as $row) { ?>
                <div style="" class="show_img"><img alt="Color" src="uploads/san-pham/<?php echo isset($row->img) ? $row->img : '' ?>" height="50" width="70">
                    <a href="back/item/delete_img/<?php echo $type . "/" . $item->id . "/" . $row->img ?>"
